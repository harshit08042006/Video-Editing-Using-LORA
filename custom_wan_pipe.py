...
class WanVideoPipeline(BasePipeline):
    ...
    def encode_image(self, image, pseudo_video_path=None, mask_video_path=None, end_image=None, num_frames=81, height=480, width=832):
        ...
        if pseudo_video_path is not None and mask_video_path is not None:
            ...
            # 3. Binarize mask matching training logic
            mask = (mask + 1) / 2              # normalize [-1,1] → [0,1]
            mask = (mask < 0.5).float()         # black=1 (preserve), white=0 (generate)
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    # Patchify and extract f, h, w from input shape and patch size
    x = dit.patchify(x)
    # Expect x after patchify to have shape (batch, dim, f, h, w).
    # Some model ops (LayerNorm) expect the feature dimension to be the last
    # dimension, so move channel/feature dim to the end and flatten spatial
    # patches into the sequence dimension: (b, dim, f, h, w) -> (b, f*h*w, dim)
    pf, ph, pw = dit.patch_size
    if x.dim() == 5:
        b, dim, f, h, w = x.shape
        x = x.permute(0, 2, 3, 4, 1).contiguous().view(b, f * h * w, dim)
    else:
        # If already in (b, seq, dim) format, try to infer f,h,w from patch size
        if x.dim() == 3:
            b, seq, dim = x.shape
            # try to recover f,h,w using known patch counts if possible
            try:
                f = dit.freqs[0].shape[0]
                h = dit.freqs[1].shape[0]
                w = seq // (f * h)
                if f * h * w != seq:
                    f = h = w = None
            except Exception:
